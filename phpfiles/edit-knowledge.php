...
if ($db->connect_error) {
            ...
            if (in_array($pdf_ex_lc, $allowed_exs)) {
                ...
                if ($stmt->execute()) {
                    echo '<script type="text/javascript">
                            window.location = "../knowledge_shared.php?alert=editsuccess";
                        </script>';
                } else {
                    echo "
    ...
    $stmt = $db->prepare("UPDATE knowledge_sharing SET username=?, title=?, department=?, post_to=?, content=?, validity=?, minimum_time=?, message=?, format=?, status=?, time_added=CURRENT_TIMESTAMP WHERE knowledge_id = ?");
    $stmt->bind_param("ssssssssssi", $username, $title, $department, $post_to, $content, $validity, $minimum_time, $message, $content_format, $status, $knowledge_id);
